...
import { generateGeneratedAppRepairProposal, getGeneratedRepairProviderStatus, } from "../ai/generatedRepairProvider.js";
...
export async function startGeneratedApp(projectId) {
    ...
    const productionBuildCompleteMarker = path.join(RUNTIME_ROOT, "data", "runtime-apps", projectId, "production-build-complete");
    ...
    if (!reusableAppDir) {
        try {
            fs.rmSync(productionBuildCompleteMarker, {
                force: true,
            });
        }
        catch {
    ...
    child.once("exit", (code, signal) => {
        ...
        // boundary instead.
        const buildCompleted = reusableAppDir ||
            fs.existsSync(productionBuildCompleteMarker);
        const failedDuringBuild = code !== 0 &&
            !buildCompleted;
...
export async function getGeneratedAppStatus(projectId) {
    ...
    // exists.
    const productionBuildCompleteMarker = path.join(RUNTIME_ROOT, "data", "runtime-apps", projectId, "production-build-complete");
    const buildCompleted = fs.existsSync(productionBuildCompleteMarker);
...
export async function orchestrateGeneratedAppRepair(projectId) {
    const history = getGeneratedAppRepairHistory(projectId);
    ...
    if (Number(pending.attempt || 0) >
        GENERATED_APP_MAX_REPAIR_ATTEMPTS) {
        return {
            ok: false,
            projectId,
            status: "repair-limit-exceeded",
    ...
    if (pending?.diagnosis
        ...
        return {
            ok: false,
            projectId,
            status: "repair-not-eligible",
            attempt: pending.attempt,
        };
    }
    const providerStatus = getGeneratedRepairProviderStatus();
    if (!providerStatus ||
        providerStatus.configured !== true) {
        return {
            ok: false,
            projectId,
            status: "repair-provider-unavailable",
            attempt: pending.attempt,
    ...
    if (!fs.existsSync(artifactDir)) {
        return {
            ok: false,
            projectId,
            status: "artifact-directory-missing",
            attempt: pending.attempt,
        };
    }
    const requestedFiles = new Set();
    const failingFile = String(pending?.diagnosis
        ?.failingFile || "").trim();
    if (failingFile) {
        try {
            requestedFiles.add(normalizeGeneratedRepairPath(failingFile));
        }
        catch {
    ...
    for (const candidate of [
        ...
        try {
            const target = resolveGeneratedRepairPath(artifactDir, candidate);
            if (fs.existsSync(target.target)) {
                requestedFiles.add(target.normalized);
            }
        }
        catch {
    ...
    const files = [];
    for (const relativeFile of requestedFiles) {
        let target;
        try {
            target =
                resolveGeneratedRepairPath(artifactDir, relativeFile);
        }
        catch {
            continue;
        }
        ...
        files.push({
            ...
            content: fs.readFileSync(target.target, "utf8"),
        });
    }
    if (files.length === 0) {
        return {
            ok: false,
            projectId,
            status: "repair-source-context-unavailable",
            attempt: pending.attempt,
        };
    }
    const planningAt = new Date().toISOString();
    const currentManifest = getGeneratedAppManifest(projectId);
    persistGeneratedAppLifecycle(projectId, Number(currentManifest?.pid || 0), {
        status: "repair-planning",
        repairAttempt: pending.attempt,
        repairProvider: providerStatus.provider,
        repairModel: providerStatus.model,
        repairPlanningStartedAt: planningAt,
        checkedAt: planningAt,
    });
    let proposal;
    try {
        proposal =
        ...
        persistGeneratedAppLifecycle(projectId, Number(getGeneratedAppManifest(projectId)?.pid || 0), {
            status: "failed",
            buildFailed: true,
            failurePhase: "build",
            failureReason: "generated-repair-provider-failed",
            repairAttempt: pending.attempt,
            repairPlanningFailed: true,
            repairPlanningError: String(error),
            failedAt,
            checkedAt: failedAt,
        });
        return {
    ...
    try {
        applied =
            applyGeneratedAppRepairProposal(projectId, proposal);
    }
    catch (error) {
        const failedAt = new Date().toISOString();
        persistGeneratedAppLifecycle(projectId, Number(getGeneratedAppManifest(projectId)?.pid || 0), {
            ...
            buildFailed: true,
            failurePhase: "build",
            failureReason: "generated-repair-executor-failed",
            repairAttempt: pending.attempt,
            repairExecutionFailed: true,
            repairExecutionError: String(error),
            failedAt,
            checkedAt: failedAt,
        });
        return {
