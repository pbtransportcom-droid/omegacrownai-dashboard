...
import { buildFinancePlatformArtifacts } from "./finance-platform-builder.js";
...
async function finalizeGeneratedArtifactRoot(artifacts, outDir, promptContract = null) {
    const resolvedArtifacts = await Promise.resolve(artifacts);
    const records = Array.isArray(resolvedArtifacts)
        ? resolvedArtifacts
        : [];
    ...
    if (promptContract &&
        ...
        enforcePromptContractArtifacts(records, promptContract);
        ...
        for (const record of records) {
            const relativeFile = String(record?.file || "").trim();
            if (!relativeFile ||
    ...
    const nextConfigPath = path.join(outDir, "next.config.mjs");
...
function contractRouteToPageFile(route) {
...
function contractApiToFile(route) {
    const normalized = String(route || "")
        .trim()
        .replace(/[?#].*$/, "")
        .replace(/^\/+|\/+$/g, "");
...
function contractResourceName(route) {
    const normalized = String(route || "")
        .trim()
        .replace(/[?#].*$/, "")
        .replace(/^\/+|\/+$/g, "");
...
function ensureContractPersistenceSupport(files) {
    const file = "lib/contract-store.ts";
    if (files.some((entry) => entry.file === file)) {
        return;
    }
    files.push({
        type: "typescript",
        title: "Contract Persistence Store",
        file,
        content: `import fs from "node:fs";
import path from "node:path";
...
function ensureContractPage(files, page, productName, workflows, features) {
    const route = String(page?.route || "").trim();
    if (!route.startsWith("/")) {
        return;
    }
    const file = contractRouteToPageFile(route);
    if (files.some((entry) => entry.file === file)) {
        return;
    }
    const name = String(page?.name ||
        contractTitle(route)).trim();
    const relatedWorkflows = workflows
        .filter((workflow) => {
        const haystack = `${workflow?.name || ""} ${Array.isArray(workflow?.steps)
            ? workflow.steps.join(" ")
            : ""}`.toLowerCase();
        const needles = [
            name,
    ...
    const relatedFeatures = features
        .filter((feature) => {
        const source = String(feature)
            .toLowerCase();
        const tokens = `${name} ${route}`
            .toLowerCase()
            .split(/[^a-z0-9]+/)
            .filter((token) => token.length >= 4);
        ...
        .slice(0, 6);
    const workflowMarkup = relatedWorkflows.length
        ? relatedWorkflows
            .map((workflow) => `<article className="contract-card">
                <span>Workflow</span>
                <h2>${String(workflow.name || "").replace(/[<>]/g, "")}</h2>
                <ol>
                  ${Array.isArray(workflow.steps)
            ? workflow.steps
                .map((step) => `<li>${String(step || "").replace(/[<>]/g, "")}</li>`)
                .join("")
            : ""}
                </ol>
              </article>`)
    ...
    const featureMarkup = relatedFeatures.length
        ? relatedFeatures
            .map((feature) => `<li>${String(feature).replace(/[<>]/g, "")}</li>`)
            .join("")
  ...
  title: ${JSON.stringify(`${name} | ${productName}`)},
};
...
function ensureContractApi(files, api) {
    const route = String(api?.route || "").trim();
    const file = contractApiToFile(route);
    if (!file) {
        return;
    }
    if (files.some((entry) => entry.file === file)) {
    ...
    ensureContractPersistenceSupport(files);
    const resource = contractResourceName(route);
    const methods = new Set(Array.isArray(api?.methods)
        ? api.methods.map((value) => String(value)
            .toUpperCase())
        : [
            "GET",
            "POST"
...
`;
  ...
  return NextResponse.json({
    ...
    if (methods.has("POST")) {
        handlers.push(`export async function POST(
  request: Request
) {
  try {
    const body =
  ...
  return NextResponse.json({
    ...
    files.push({
        type: "typescript",
        title: `${contractTitle(resource)} API Route`,
        file,
        content: imports +
            "\n" +
            handlers.join("\n\n") +
...
function ensureContractPrismaModels(files, models) {
    if (!models.length) {
        return;
    }
    let schema = files.find((entry) => entry.file ===
...
`,
    ...
    let content = String(schema.content || "");
    for (const model of models) {
        const name = String(model?.name || "").trim();
        if (!/^[A-Z][A-Za-z0-9]*$/.test(name)) {
            continue;
        }
        const exists = new RegExp(`\\bmodel\\s+${name}\\s*\\{`).test(content);
        if (exists) {
            continue;
        }
        content +=
...
function enforcePromptContractArtifacts(files, promptContract) {
    if (!promptContract ||
        typeof promptContract !==
            "object") {
        return;
    }
    const pages = Array.isArray(promptContract.pages)
        ? promptContract.pages
        : [];
    const apiRoutes = Array.isArray(promptContract.apiRoutes)
        ? promptContract.apiRoutes
        : [];
    const models = Array.isArray(promptContract.models)
        ? promptContract.models
        : [];
    const workflows = Array.isArray(promptContract.workflows)
        ? promptContract.workflows
        : [];
    const features = Array.isArray(promptContract.features)
        ? promptContract.features.map((value) => String(value))
        : [];
    const productName = String(promptContract.productName ||
        "Generated Application").trim();
    for (const page of pages) {
        ensureContractPage(files, page, productName, workflows, features);
    }
    for (const api of apiRoutes) {
        ensureContractApi(files, api);
    }
    ensureContractPrismaModels(files, models);
    const metadata = files.find((entry) => entry.file ===
        "metadata.json");
    if (metadata) {
        try {
            ...
            parsed.requiredRoutes =
                pages.map((page) => contractRouteToPageFile(page.route));
            parsed.requiredApiRoutes =
                apiRoutes.map((api) => contractApiToFile(api.route));
...
function detectPromptMode(prompt, fallbackMode) {
...
function profileName(mode) {
    return String(mode || "Production")
        .replace(/[-_]+/g, " ")
        .replace(/\b\w/g, (char) => char.toUpperCase());
}
function domainProfile(mode) {
    const profiles = {
        trading: {
            navPrimary: "Signals",
...
function modeProfile(mode, prompt, isTransport) {
    ...
    const profiles = {
        ...
        app: {
            ...
            navSecondary: "Customers",
            navTertiary: "Records",
            cta: "Open App",
            heroPrimary: "Review App Dashboard",
            heroSecondary: "View Records",
            featureTitle: "Business App Modules",
            featureFile: "components/DashboardModules.tsx",
            featureComponent: "DashboardModules",
            actionTitle: "Customer Record Component",
            actionFile: "components/CustomerRecordForm.tsx",
            actionComponent: "CustomerRecordForm",
            actionHeading: "Create customer record",
            actionDescription: "Capture customer details, request type, status, notes, priority, owner, and follow-up workflow.",
            areaTitle: "Business App Workspace",
            areaHeading: "Dashboard, customer records, admin review, status tracking, data store, API routes, and launch support",
            testimonialTitle: "Built for daily business operations",
            adminTitle: "Business App Admin Dashboard",
            leadModel: "CustomerRecord",
            leadFile: "customer-records.json",
            notificationEnv: "APP_NOTIFICATION_EMAIL",
            smokeService: "Business App",
            modeItemOne: "Customer Records",
            modeItemTwo: "Admin Review",
            modeItemThree: "Status Tracking",
        },
        automation: {
            navPrimary: "Workflows",
            navSecondary: "Triggers",
            navTertiary: "Run History",
            cta: "Build Workflow",
            heroPrimary: "Review Automation Dashboard",
            heroSecondary: "View Trigger Map",
            featureTitle: "Automation Builder Modules",
            featureFile: "components/WorkflowMap.tsx",
            featureComponent: "WorkflowMap",
            actionTitle: "Automation Request Component",
            actionFile: "components/WorkflowIntakeForm.tsx",
            actionComponent: "WorkflowIntakeForm",
            actionHeading: "Create automation request",
            actionDescription: "Capture trigger event, action steps, owner, connected system, approval rule, status, and success criteria.",
            areaTitle: "Workflow Automation System",
            areaHeading: "Workflow dashboard, trigger/action map, request intake, admin review, status tracking, run history, and launch package",
            testimonialTitle: "Built for repeatable business operations",
            adminTitle: "Automation Admin Dashboard",
            leadModel: "AutomationRequest",
            leadFile: "automation-requests.json",
            notificationEnv: "AUTOMATION_NOTIFICATION_EMAIL",
            smokeService: "Automation Builder",
            modeItemOne: "Trigger Actions",
            modeItemTwo: "Admin Review",
            modeItemThree: "Run History",
        },
        marketing: {
...
export async function buildArtifacts(run) {
    ...
    const promptContract = buildSpec?.promptContract ||
        run.promptContract ||
        null;
    ...
    // AUTHORITATIVE_FINANCE_FROM_SPEC
    // The finalized build-spec industry is authoritative.
    // Finance capabilities such as payments, accounts, Stripe,
    // Square, checkout, reporting, or billing must not reclassify
    // another product family as Finance.
    ...
    const authoritativeBlueprint = run.buildSpec?.authoritativeBlueprint;
    if (authoritativeBlueprint) {
        const livingOSPlan = createLivingOSProductionPlan(authoritativeBlueprint);
        const composition = composeLivingOSApplication(livingOSPlan);
        if (composition.supported) {
            const records = [];
            const livingOSFiles = [...composition.files];
            const existingLivingOSFiles = new Set(livingOSFiles.map((file) => file.file));
            const productionScaffold = [
                {
                    ...
                    content: JSON.stringify({
                        name: String(run?.projectId ||
                            "omegacrownai-generated-application").toLowerCase(),
                        version: "1.0.0",
                        private: true,
                        // FINANCE_LIVING_OS_PACKAGE_INTEGRATION
                        scripts: {
                            dev: "next dev",
                        ...
                        ...(financeFromSpec
                            ? {
                                author: "Princess Benjamin Transportation Company",
                                license: "UNLICENSED",
                                omegacrownai: {
                                    generator: "OmegaCrownAI",
...
`,
            ...
            // BOOKSTORE_LIVING_OS_DELIVERY_INTEGRATION
            // Bookstore deployable artifacts expose company-level ownership
            // and production guidance without Finance-specific terminology,
            // personal founder information, secrets, or internal infrastructure.
            if (specIndustry === "bookstore") {
                productionScaffold.push({
                    file: ".env.example",
                    title: "Bookstore Environment Template",
                    type: "text",
                    content: `# BookHaven environment template
# Replace placeholder values in the deployment environment.
# Never commit live production credentials.

DATABASE_URL="postgresql://bookhaven_user:CHANGE_ME@db-host:5432/bookhaven?schema=public"
NEXT_PUBLIC_APP_NAME="${run?.productName ||
                        run?.buildSpec?.brandName ||
                        "BookHaven"}"

# Optional production payment integrations
STRIPE_SECRET_KEY=""
SQUARE_ACCESS_TOKEN=""
`,
                }, {
                    file: "README.md",
                    title: "BookHaven Production Delivery Guide",
                    type: "markdown",
                    content: `# ${run?.productName ||
                        run?.buildSpec?.brandName ||
                        "BookHaven"}

Production-ready bookstore application generated by OmegaCrownAI.
...
Personal founder information is intentionally excluded from deployable application metadata.

## Included Bookstore capabilities

- Book catalog and browsing
- Category and author discovery
- Search and filtering
- Hardcover, paperback, ebook, and audiobook formats
- Shopping cart
- Wishlist and save-for-later
- Secure Stripe or Square checkout contract
...
- Prisma-backed production database contract
- Generated smoke verification
...
Configure Stripe or Square production credentials only when those providers are ready to be activated.

Do not place production passwords, API keys, session tokens, private keys, internal server paths, or other secrets in source control.
...
Apply the database schema using the approved deployment workflow.
...
Run generated application smoke testing:
...
Provide DATABASE_URL and payment-provider credentials through the deployment environment or approved secret manager.

Verify digital delivery, shipping, order confirmation, customer account access, and payment-provider configuration before production launch.

## Security and privacy

This delivery package does not intentionally include:
...
Sensitive production configuration belongs in the deployment environment or an approved secrets manager.
`,
                }, {
                    file: "DELIVERY.md",
                    title: "Bookstore Production Delivery",
                    type: "markdown",
                    content: `# Bookstore Production Delivery

Generated by OmegaCrownAI for Princess Benjamin Transportation Company.

## Required production checks

1. Configure DATABASE_URL securely.
2. Run npm run db:generate.
3. Apply the production database schema using the approved deployment workflow.
4. Run npm run build.
5. Run npm run smoke.
6. Verify bookstore search, cart, wishlist, checkout, accounts, digital delivery, and shipping.
7. Confirm Stripe or Square credentials only when production payment processing is ready.
8. Verify ebook and audiobook delivery permissions.
9. Verify physical shipping and tracking behavior.
10. Confirm backups, monitoring, logging, and recovery procedures.
11. Keep all production secrets outside source control.

No personal founder information or internal infrastructure credentials are required in this deployable package.
`,
                }, {
                    file: "LAUNCH_CHECKLIST.md",
                    title: "Bookstore Launch Checklist",
                    type: "markdown",
                    content: `# Bookstore Launch Checklist

- [ ] Production DATABASE_URL configured securely
- [ ] Prisma client generated
- [ ] Production database schema applied
- [ ] Production build passes
- [ ] Generated smoke test passes
- [ ] Book catalog reviewed
- [ ] Book search tested
- [ ] Cart and wishlist tested
- [ ] Hardcover and paperback ordering tested
- [ ] Ebook delivery tested
- [ ] Audiobook delivery tested
- [ ] Stripe or Square configuration reviewed
- [ ] Shipping quote and tracking flows tested
- [ ] Customer account and order history tested
- [ ] Subscription workflow tested
- [ ] Reviews and ratings workflow tested
- [ ] Admin inventory and order management reviewed
- [ ] Promotions and discounts reviewed
- [ ] Backups configured
- [ ] Monitoring configured
- [ ] No secrets in source artifacts
- [ ] No internal server paths in public artifacts
- [ ] Customer-facing branding reviewed
`,
                });
            }
            // FINANCE_LIVING_OS_DELIVERY_INTEGRATION
            // Public/deployable artifacts intentionally expose company-level
            // ownership only. Personal founder information, internal server
            // paths, credentials, runtime topology, and secrets are excluded.
            if (financeFromSpec) {
                productionScaffold.push({
                    file: ".env.example",
                    title: "Finance Environment Template",
                    type: "text",
                    content: `# CrownLedger environment template
# Replace placeholder values in the deployment environment.
# Never commit live production credentials.

DATABASE_URL="postgresql://crownledger_user:CHANGE_ME@db-host:5432/crownledger?schema=public"
NEXT_PUBLIC_APP_NAME="${run?.productName ||
                        run?.buildSpec?.brandName ||
                        "CrownLedger"}"
`,
                    ...
                    file: "README.md",
                    title: "CrownLedger Finance Delivery Guide",
                    type: "markdown",
                    content: `# ${run?.productName ||
                        run?.buildSpec?.brandName ||
                        "CrownLedger"}

Production-ready Finance application generated by OmegaCrownAI.
...
- Financial dashboard and account operations
- Transaction monitoring and transaction-detail CRUD
...
Seed starter/demo data when appropriate:
...
Run Finance full-stack structural verification:
...
Provide DATABASE_URL through the deployment environment or secret manager.

Run the database schema operation before first production launch.

Seed data is optional and should only be used when starter/demo records are appropriate.

Review production authentication, authorization, payment-provider credentials, database backups, logging, monitoring, and recovery controls before accepting real financial activity.
...
`,
                    ...
                    file: "DELIVERY.md",
                    title: "Finance Production Delivery",
                    type: "markdown",
                    content: `# Finance Production Delivery
...
4. Run npm run build.
5. Run npm run smoke.
6. Run npm run test:fullstack.
7. Verify authentication and server-side authorization before handling real financial information.
8. Confirm backups, monitoring, logging, and recovery procedures.
9. Keep all production secrets outside source control.

No personal founder information or internal infrastructure credentials are required in this deployable package.
`,
                }, {
                    file: "LAUNCH_CHECKLIST.md",
                    title: "Finance Launch Checklist",
                    type: "markdown",
                    content: `# Finance Launch Checklist

- [ ] Production DATABASE_URL configured securely
- [ ] Prisma client generated
- [ ] Database schema applied
- [ ] Starter seed reviewed before use
- [ ] Production build passes
- [ ] Generated smoke test passes
- [ ] Finance full-stack test passes
- [ ] Authentication implemented and tested
- [ ] Protected pages enforce server-side authorization
- [ ] Forged and revoked sessions rejected
- [ ] Production payment credentials stored outside source
- [ ] Backups configured
- [ ] Monitoring configured
...
`,
            ...
            for (const file of livingOSFiles) {
                const target = path.join(outDir, file.file);
    ...
    if (!automationFromSpec && saasFromSpec) {
        return finalizeGeneratedArtifactRoot(buildSaasLandingArtifacts(run, outDir), outDir, promptContract);
    }
    if (legalFromSpec) {
        return finalizeGeneratedArtifactRoot(buildLegalFirmArtifacts(run, outDir), outDir, promptContract);
    }
    if (tradingFromSpec) {
        return finalizeGeneratedArtifactRoot(buildTradingPlatformArtifacts(run, outDir), outDir, promptContract);
    }
    if (restaurantFromSpec) {
        return finalizeGeneratedArtifactRoot(buildRestaurantPlatformArtifacts(run, outDir), outDir, promptContract);
    }
    if (financeFromSpec) {
        return finalizeGeneratedArtifactRoot(buildFinancePlatformArtifacts(run), outDir, promptContract);
    }
    if (isUniversalAnythingPrompt(run.prompt || "") && !transportFromSpec) {
        return finalizeGeneratedArtifactRoot(buildUniversalAnythingArtifacts(run, outDir), outDir, promptContract);
    }
...
`
    ...
    if (isTransport) {
        const transportSvgAsset = (label, description, bg, fg) => `<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="900" viewBox="0 0 1600 900">
  ...
  <text x="160" y="455" font-family="Arial, sans-serif" font-size="52" font-weight="700" fill="#f8fafc">${description}</text>
        ...
        const upsertFile = (file, title, content, type = "typescript") => {
...
`);
    ...
    const generatedNextConfig = files.find((entry) => entry.file ===
        "next.config.mjs");
...
`,
    ...
    enforcePromptContractArtifacts(files, promptContract);
