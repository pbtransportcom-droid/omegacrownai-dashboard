...
export function renderProfessionalServicesLivingOS(plan) {
    const brand = safe(plan.business.brandName) ||
    ...
    files.push({
        ...
        content: JSON.stringify({
            ...
            valueProposition: plan.business.valueProposition,
            conversionGoals: plan.business.conversionGoals,
        }, null, 2),
    });
    files.push({
...
const navigation = ${JSON.stringify(plan.pages
    ...
    <section className="executive-hero">
      <div>
        ...
        <p className="hero-copy">
          ${safe(plan.business.conversionGoals.join(" "))}
        </p>
...
`,
    ...
    files.push({
        ...
        type: "typescript",
        content: `const modules = ${JSON.stringify(plan.modules.map((module) => ({
            name: module.name,
...
`,
    ...
    files.push({
        ...
        type: "typescript",
        content: `const workflows = ${JSON.stringify(plan.workflows, null, 2)};
...
import {
  createRecord,
  listRecords,
} from "${"../".repeat(Math.max(3, apiFile(route)
                .split("/")
                .length - 2))}lib/runtime-store";

const collection = ${JSON.stringify(slug(route) || "records")};
...
`,
    ...
    const modelBlocks = plan.architecture.dataModels
        .map((model) => {
...
`,
    ...
    files.push({
        ...
        type: "typescript",
        content: `const modules = ${JSON.stringify(plan.architecture.adminModules, null, 2)};
...
`,
    ...
    for (const file of files) {
        ...
        file.content = file.content
            .split("prospective legal clients and case intake leads")
            .join("professional services clients and prospective engagement leads")
            .split("Legal Authority Website OS")
            .join(brand)
            .split("Legal Authority Website Admin Dashboard")
            .join(brand + " Admin Dashboard");
        if (!protectedProfessionalPageFiles.has(file.file)) {
            continue;
        }
        if (file.content.includes("requireProfessionalSession")) {
            continue;
        }
        const functionMatch = file.content.match(/export default function\s+([A-Za-z0-9_]+)\s*\(\)\s*\{/);
        if (!functionMatch) {
            throw new Error("Unable to protect generated professional-services page: " +
                file.file);
        }
...
` +
                file.content;
        file.content =
            file.content.replace(functionMatch[0], `export default async function ${functionName}() {
  try {
    ...
    for (const file of files) {
        file.content = file.content
            ...
            .split("Legal Authority Website OS")
            .join(brand)
            .split("Custom Business Website Admin Dashboard")
            .join(brand + " Admin Dashboard")
            .split("Custom Business Website OS")
            .join(brand);
    }
    return files;
}
