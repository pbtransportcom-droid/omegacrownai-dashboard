...
function normalizePath(artifact) {
    return String(artifact.file ||
        artifact.path ||
        artifact.title ||
        "")
        .replace(/\\/g, "/")
...
function findArtifact(artifacts, file) {
    ...
    return artifacts.find((artifact) => {
        const path = normalizePath(artifact);
        return (path === wanted ||
            path.endsWith("/" + wanted));
    });
}
function inspect(artifacts, files, terms) {
    const evidenceFiles = [];
    ...
    const missingEvidence = terms.filter((term) => !foundTerms.has(term));
    return {
...
function requirement(artifacts, input) {
    const result = inspect(artifacts, input.files, input.terms);
    return {
        id: input.id,
        label: input.label,
...
function commerceRequirements(artifacts) {
    return [
        ...
        requirement(artifacts, {
            ...
            label: "Commerce persistence model",
            files: [
...
function restaurantRequirements(artifacts) {
    return [
        requirement(artifacts, {
            id: "restaurant-reservations",
            label: "Table reservation workflow",
            files: [
        ...
        requirement(artifacts, {
            ...
            label: "Online ordering workflow",
            files: [
...
function saasRequirements(artifacts) {
    return [
        requirement(artifacts, {
            id: "saas-workspace",
            label: "Multi-tenant workspace state",
            files: [
...
function automationRequirements(artifacts) {
    return [
        requirement(artifacts, {
            id: "automation-builder",
            label: "Workflow builder execution",
            files: [
...
function generalBusinessRequirements(artifacts) {
    return [
        requirement(artifacts, {
...
function legalRequirements(artifacts) {
    return [
        requirement(artifacts, {
            id: "legal-intake",
...
function healthcareRequirements(artifacts) {
    return [
        requirement(artifacts, {
...
function bookstoreRequirements(artifacts) {
    return [
        requirement(artifacts, {
            id: "bookstore-cart",
            label: "Bookstore commerce state",
            files: [
                "components/CommerceProvider.tsx",
                "app/cart/page.tsx",
...
export function evaluateBehavioralCompliance(industry, artifacts) {
    ...
    switch (normalizedIndustry) {
        ...
        case "transportation":
            requirements =
                transportationRequirements(inspectable);
            break;
        case "legal":
            requirements =
                legalRequirements(inspectable);
            break;
        case "healthcare":
            requirements =
                healthcareRequirements(inspectable);
            break;
        case "bookstore":
            requirements =
                bookstoreRequirements(inspectable);
            break;
        case "restaurant":
            requirements =
                restaurantRequirements(inspectable);
            break;
        case "commerce":
            requirements =
                commerceRequirements(inspectable);
            break;
        case "saas":
            requirements =
                saasRequirements(inspectable);
            break;
        case "finance":
            requirements =
                financeRequirements(inspectable);
            break;
        case "automation":
            requirements =
                automationRequirements(inspectable);
            break;
        default:
            requirements = [];
            break;
    }
    if (specializedIndustries.has(normalizedIndustry) &&
        requirements.length === 0) {
        requirements = [
            {
                id: "missing-behavioral-contract",
                label: `Behavioral contract for ${normalizedIndustry}`,
                required: true,
                passed: false,
                evidenceFiles: [],
                missingEvidence: [
    ...
    const score = required.length === 0
        ? 100
        : Math.round((passed.length /
            required.length) *
            100);
    const failedRequirements = required
        .filter((item) => !item.passed)
        .map((item) => item.label);
    const deliveryBlocked = failedRequirements.length > 0;
    return {
