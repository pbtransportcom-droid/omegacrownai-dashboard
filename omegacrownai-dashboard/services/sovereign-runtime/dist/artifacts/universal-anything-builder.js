...
function inferDomain(prompt, buildSpec) {
    const explicitIndustry = cleanPrompt(buildSpec?.industry || "").toLowerCase();
    const explicitProductType = cleanPrompt(buildSpec?.productType || "");
    const source = [
        explicitIndustry,
    ...
    const profiles = [
        {
            ...
            profile: {
                ...
                conversionGoal: "Turn travelers and corporate clients into confirmed transportation bookings.",
                valueProposition: "Reliable, professionally managed transportation with clear booking, fleet, and dispatch visibility.",
                sectionSequence: [
                    "Booking-first hero",
                    "Service coverage",
                    "Fleet options",
        ...
        },
        {
            ...
            profile: {
                ...
                conversionGoal: "Convert urgent and planned plumbing needs into qualified service requests.",
                valueProposition: "Fast, trustworthy plumbing support with clear services, emergency response, and quote follow-up.",
                sectionSequence: [
        ...
        },
        {
            ...
            profile: {
                ...
                conversionGoal: "Convert prospective clients into qualified consultation requests.",
                valueProposition: "Clear legal guidance backed by credible attorney experience and structured case intake.",
                sectionSequence: [
                    "Authority-led hero",
                    "Practice areas",
        ...
        },
        {
            ...
            profile: {
                ...
                conversionGoal: "Convert visitors into reservations, orders, and catering inquiries.",
                valueProposition: "An inviting dining experience with convenient reservations, ordering, and event catering.",
                sectionSequence: [
                    "Food-led hero",
                    "Signature dishes",
                    "Menu highlights",
        ...
        },
        {
            ...
            profile: {
                ...
                conversionGoal: "Convert campaign visitors into qualified leads and measurable follow-up actions.",
                valueProposition: "A coordinated campaign experience that connects the offer, lead capture, content, and review workflow.",
                sectionSequence: [
                    "Offer-first hero",
                    "Audience problem",
                    "Campaign promise",
        ...
        },
        {
            ...
            profile: {
                ...
                conversionGoal: "Turn operational requirements into reviewable and trackable automated workflows.",
                valueProposition: "Clear workflow automation from trigger definition through execution history and admin review.",
                sectionSequence: [
    ...
    return {
        ...
        conversionGoal: "Turn qualified visitors into clear customer actions.",
        valueProposition: "A business-specific digital experience built around the requested services and operating workflow.",
...
function cleanBlueprintRoute(route) {
...
function relativeLibImport(file) {
...
function tokenizeRequirement(value) {
    return String(value || "")
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, " ")
        .split(/\s+/)
...
function requirementEvidenceFiles(files, requirement) {
...
function createBlueprintPageFile(page, route, brand, blueprint) {
    const file = pageFileFromRoute(route);
    const componentName = safeIdentifier(page?.name, "GeneratedPage");
    ...
    const qualityLevel = JSON.stringify(blueprint?.design?.qualityLevel || "standard");
    const productType = JSON.stringify(blueprint?.business?.productType ||
...
function createBlueprintStoreFile() {
    return {
...
function createBlueprintApiFile(route, featureName) {
...
function evaluateBlueprintEvidence(files, blueprint) {
    const pageEvidence = (Array.isArray(blueprint?.pages)
        ? blueprint.pages
        : []).map((page) => {
        const route = blueprint?.architecture?.routes?.[blueprint.pages.indexOf(page)] ||
            "/" +
                String(page?.name || "")
                    .toLowerCase()
                    .replace(/[^a-z0-9]+/g, "-")
        ...
        return {
            ...
            route,
            file,
            implemented: files.some((candidate) => candidate.file === file),
        };
    });
    const apiEvidence = (Array.isArray(blueprint?.architecture?.apiRoutes)
        ...
        return {
            route,
            file,
            implemented: Boolean(file) &&
                files.some((candidate) => candidate.file === file),
        };
    });
    const featureEvidence = (Array.isArray(blueprint?.features)
        ? blueprint.features
        : []).map((feature) => {
        const evidenceFiles = requirementEvidenceFiles(files, String(feature?.name || ""));
        return {
    ...
    const workflowEvidence = (Array.isArray(blueprint?.workflows)
        ? blueprint.workflows
        : []).map((workflow) => {
        const workflowText = [
            workflow?.name,
            ...
            .filter(Boolean)
            .join(" ");
        const evidenceFiles = requirementEvidenceFiles(files, workflowText);
        return {
    ...
    const searchableOutput = files
        ...
        .join("\n")
        .toLowerCase();
    const prohibitedPatternMatches = (blueprint?.design?.prohibitedPatterns || []).filter((pattern) => searchableOutput.includes(String(pattern).toLowerCase()));
    const requestedQualityLevel = blueprint?.design?.qualityLevel || "standard";
    const premiumLanguagePresent = requestedQualityLevel === "standard" ||
        /premium|luxury|executive|refined|polished|high-end/.test(searchableOutput);
    const designSystemPresent = files.some((file) => file.file === "app/globals.css" ||
    ...
    const score = totalChecks > 0
        ...
        : 0;
    const minimumQualityScore = Number(blueprint?.compliance?.minimumQualityScore || 88);
...
function appendBlueprintDrivenArtifacts(files, blueprint, brand) {
    if (!blueprint)
        return null;
    if (!files.some((file) => file.file === "lib/blueprint-store.ts")) {
        files.push(createBlueprintStoreFile());
    }
    const blueprintPages = Array.isArray(blueprint.pages)
        ? blueprint.pages
        : [];
    const blueprintRoutes = Array.isArray(blueprint?.architecture?.routes)
        ? blueprint.architecture.routes
        : [];
    blueprintPages.forEach((page, index) => {
        ...
        if (!files.some((candidate) => candidate.file === file)) {
            files.push(createBlueprintPageFile(page, route, brand, blueprint));
        }
    });
    ...
    apiRoutes.forEach((route, index) => {
        ...
        if (!file)
            return;
        if (!files.some((candidate) => candidate.file === file)) {
            const featureName = blueprint?.features?.[index]?.name ||
                route.split("/").filter(Boolean).at(-1) ||
                "Business record";
            files.push(createBlueprintApiFile(route, featureName));
        }
    });
    files.push({
    ...
    const evidence = evaluateBlueprintEvidence(files, blueprint);
    files.push({
...
export async function buildUniversalAnythingArtifacts(run, outDir) {
    ...
    const authoritativeBlueprint = buildSpec?.authoritativeBlueprint || null;
    ...
    const featureItems = (specFeatures.length ? specFeatures : domain.sections).slice(0, 6).map((item, index) => ({
        title: item,
    ...
    const workflowItems = [
        ...specCustomerWorkflow.slice(0, 4).map((item) => ({ role: "Customer", item })),
        ...specAdminWorkflow.slice(0, 4).map((item) => ({ role: "Admin", item }))
    ];
    const ctaLabel = buildSpec?.industry === "beauty" ? "Book Appointment" :
            ...
            buildSpec?.industry === "clinic" ? "Request Appointment" :
                buildSpec?.industry === "commerce" ? "Start Checkout" :
                    buildSpec?.industry === "legal" ? "Request Consultation" :
                        "Submit Request";
    const intakeTitle = buildSpec?.industry === "beauty" ? "Appointment Booking" :
            ...
            buildSpec?.industry === "clinic" ? "Patient Intake" :
                buildSpec?.industry === "commerce" ? "Checkout Request" :
                    buildSpec?.industry === "legal" ? "Consultation Intake" :
                        "Customer Intake";
    const heroTitle = buildSpec?.industry === "beauty" ? `${brand} beauty booking, services, and gallery experience.` :
...
`
    ...
    const blueprintEvidence = appendBlueprintDrivenArtifacts(files, authoritativeBlueprint, brand);
    if (blueprintEvidence?.deliveryBlocked) {
        const metadataFile = files.find((file) => file.file === "metadata.json");
        if (metadataFile) {
            try {
                ...
                metadata.generatedArtifactQualityReport = {
                    ...(metadata.generatedArtifactQualityReport || {}),
                    expectedTermsPassed: blueprintEvidence.missingFeatures.length === 0,
                    frontendComplete: blueprintEvidence.missingPages.length === 0,
                    apiComplete: blueprintEvidence.missingApiRoutes.length === 0,
                    delivered: false,
                    customerReady: false,
                    deliveryBlocked: true,
                    blueprintScore: blueprintEvidence.score,
                    minimumBlueprintScore: blueprintEvidence.minimumQualityScore,
                };
