...
export async function executeRun(projectId, input) {
    ...
    try {
        ...
        const authoritativeCustomerPrompt = String(buildSpec?.originalPrompt ||
            run.prompt ||
            "").trim();
        if (!authoritativeCustomerPrompt) {
            throw new Error("Authoritative customer prompt is empty.");
        }
        //
        ...
        const deterministicPromptContract = createPromptContract(authoritativeCustomerPrompt, {
            industry: String(buildSpec?.industry ||
                ""),
            productType: String(buildSpec?.productType ||
                ""),
            productName: String(buildSpec?.productName ||
                run?.productName ||
                ""),
        });
        let authoritativePromptContract = deterministicPromptContract;
        let promptPlannerStatus = "deterministic-fallback";
        try {
            const planning = await planPromptWithAI({
                prompt: authoritativeCustomerPrompt,
                fallbackContract: deterministicPromptContract,
            });
            if (planning?.ok === true &&
                planning?.contract) {
                ...
                promptPlannerStatus =
                    "ai-reconciled";
                run.promptPlanner = {
                    status: promptPlannerStatus,
                    provider: planning.provider,
                    model: planning.model,
                };
                appendRunEvent(run, "AI product planning completed");
                appendTranscript(projectId, "AI product planning completed and reconciled with explicit customer requirements.");
            }
        }
        catch (error) {
            run.promptPlanner = {
            ...
            appendRunEvent(run, "AI product planning unavailable; deterministic contract retained");
            appendTranscript(projectId, "AI product planning unavailable. Deterministic customer requirement contract retained.");
        }
        run.originalPrompt =
        ...
        run.buildSpec = {
            ...buildSpec,
            promptContract: authoritativePromptContract,
            promptPlannerStatus,
        };
        appendRunEvent(run, `Authoritative product contract ready: ${authoritativePromptContract.pages.length} pages, ${authoritativePromptContract.apiRoutes.length} APIs, ${authoritativePromptContract.models.length} models`);
        ...
        const hasGeneratedReadme = run.artifacts.some((artifact) => {
            const artifactPath = String(artifact?.file ||
                artifact?.path ||
                "")
                .replace(/\\/g, "/")
                .trim();
            return (artifactPath === "README.md" ||
                artifactPath.endsWith("/README.md"));
        });
        if (!hasGeneratedReadme) {
            const semanticArtifact = run.artifacts.find((artifact) => {
                const artifactPath = String(artifact?.file ||
                    artifact?.path ||
                    "")
                    .replace(/\\/g, "/")
                ...
                return (artifactPath ===
                    "living-os-traceability.json" ||
                    artifactPath.endsWith("/living-os-traceability.json") ||
                    artifactPath ===
                        "living-os-plan.json" ||
                    artifactPath.endsWith("/living-os-plan.json"));
            });
            let semanticMetadata = null;
            if (semanticArtifact) {
                try {
                    const semanticRecord = semanticArtifact;
                    let rawSemantic = semanticRecord.content;
                    const semanticPath = String(semanticRecord.path || "").trim();
            ...
            const generatedIndustry = String(semanticMetadata?.industry ||
                buildSpec.industry ||
                run.mode ||
                "application")
                .trim()
                .toLowerCase();
            const generatedProduct = String(semanticMetadata?.product ||
...
`;
            const runtimeRoot = process.cwd().endsWith(path.join("services", "sovereign-runtime"))
                ? process.cwd()
                : path.resolve(process.cwd(), "services", "sovereign-runtime");
            const readmePath = path.join(runtimeRoot, "data", "artifacts", projectId, "README.md");
            ...
            appendRunEvent(run, "Generated package README finalized");
            appendTranscript(projectId, "Generated package README finalized.");
        }
