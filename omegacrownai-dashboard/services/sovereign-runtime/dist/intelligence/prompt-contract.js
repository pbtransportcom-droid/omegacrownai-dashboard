...
function contains(source, ...terms) {
    return terms.some((term) => source.includes(term.toLowerCase()));
}
export function createPromptContract(prompt, buildSpec = null) {
    ...
    for (const [route, terms] of pageRules) {
        if (contains(source, ...terms.map((term) => term.toLowerCase()))) {
            pages.push(route);
        }
    }
    const apiRoutes = [];
    ...
    const compactPrompt = originalPrompt
        .toLowerCase()
        .replace(/[^a-z0-9]/g, "");
    const models = modelRules
        .filter(([name, terms]) => {
        const compactModelName = String(name)
            .toLowerCase()
            .replace(/[^a-z0-9]/g, "");
        return (contains(source, ...terms) ||
            (compactModelName.length > 0 &&
                compactPrompt.includes(compactModelName)));
    })
        .map(([name]) => ({
        name,
        required: true
    ...
    const lines = originalPrompt
        .replace(/\r\n/g, "\n")
        .split("\n");
    for (let index = 0; index < lines.length; index += 1) {
        const line = String(lines[index] || "");
        const declaration = line.match(/^\s*(?:authentication\s+roles?|user\s+roles?|roles?)\s*:\s*(.*)$/i);
        if (!declaration) {
            continue;
        }
        const declaredChunks = [];
        const inline = String(declaration[1] || "").trim();
        if (inline) {
            declaredChunks.push(inline);
        }
        ...
        for (let cursor = index + 1; cursor < lines.length; cursor += 1) {
            const nextRaw = String(lines[cursor] || "");
            const next = nextRaw.trim();
            if (!next) {
                break;
            }
            // Another explicit section/header starts here.
            if (/^[A-Za-z][A-Za-z0-9 /_-]*:\s*$/.test(next)) {
                break;
            }
            const cleaned = next
                .replace(/^[-*•]\s*/, "")
            ...
            const normalized = cleaned
                ...
                .replace(/[^a-z0-9_-]/g, " ")
                .trim();
            const looksLikeRoleLine = normalized
                .split(/\s+/)
                .filter(Boolean)
                .every((token) => allowedRoles.includes(token) ||
                token === "and");
            if (!looksLikeRoleLine) {
                break;
            }
            declaredChunks.push(cleaned);
            index = cursor;
        }
        const declaredRoles = declaredChunks
            .join(",")
            .toLowerCase()
            .split(/[,;]|\band\b|\s+/)
            .map((value) => value
            ...
            .trim())
            .filter(Boolean);
        explicitRoleValues.push(...declaredRoles);
    }
    for (const role of allowedRoles) {
        if (explicitRoleValues.includes(role)) {
            roles.push(role);
        }
    }
    const features = [];
    ...
    const workflows = [];
    if (contains(source, "booking", "reservation")) {
        workflows.push({
    ...
    const productName = String(buildSpec?.productName ||
        buildSpec?.name ||
        "").trim() ||
        (originalPrompt.match(/\bcalled\s+([A-Z][A-Za-z0-9 ]{2,80})/i)?.[1]?.trim() ||
    ...
    return {
        ...
        productType: String(buildSpec?.productType ||
            (contains(source, "saas", "web application", "platform")
                ? "web-application"
                : "website")),
        industry: String(buildSpec?.industry ||
            "general"),
        pages: unique(pages).map((route) => ({
            route,
            name: routeName(route),
            required: true
        })),
        apiRoutes: unique(apiRoutes).map((route) => ({
            route,
            methods: [
                "GET",
                "POST"
        ...
        quality: {
            ...
            persistentDataRequired: contains(source, "persist", "persistent", "database"),
