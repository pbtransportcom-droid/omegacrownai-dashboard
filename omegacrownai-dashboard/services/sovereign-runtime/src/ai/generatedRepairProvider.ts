...
export type GeneratedRepairDiagnostic = {
  category: string;
  message: string;
...
export type GeneratedRepairContextFile = {
  file: string;
  content: string;
};

export type GeneratedRepairRequest = {
  projectId: string;
  attempt: number;
  diagnosis: GeneratedRepairDiagnostic;
  files: GeneratedRepairContextFile[];
};

export type GeneratedRepairProposal = {
  provider: string;
  model: string;
  summary: string;
  files: Array<{
...
function repairModel() {
  const value =
    String(
      process.env.OMEGACROWN_REPAIR_MODEL ||
  ...
  if (!value) {
    throw new Error(
      "OMEGACROWN_REPAIR_MODEL is not configured."
    );
  }
...
function normalizeRequest(
  request: GeneratedRepairRequest
): GeneratedRepairRequest {
  return {
    projectId:
...
function validateProposal(
  raw: any,
  model: string
): GeneratedRepairProposal {
  if (
    !raw ||
    typeof raw !== "object"
  ) {
    throw new Error(
      "Repair proposal is not an object."
    );
  }
  ...
  if (!summary) {
    throw new Error(
      "Repair proposal summary is empty."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Repair proposal has invalid file count."
    );
  }
  ...
  const normalized =
    files.map((item: any) => {
      ...
      if (!file) {
        throw new Error(
          "Repair proposal contains empty path."
        );
      }
...
export async function generateGeneratedAppRepairProposal(
  request: GeneratedRepairRequest
): Promise<GeneratedRepairProposal> {
  const key =
    apiKey();
  ...
  if (
    ...
    throw new Error(
      "Diagnosis is not repair eligible."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "No source files were supplied for repair."
    );
  }
          ...
          JSON.stringify({
            ...
            input:
              JSON.stringify(
                {
                  task:
                    "Repair generated Next.js production build failure.",
  ...
  if (!response.ok) {
    throw new Error(
      `OpenAI repair provider HTTP ${response.status}: ${
        String(
  ...
  try {
    ...
    throw new Error(
      "OpenAI repair provider returned invalid JSON."
    );
  }
