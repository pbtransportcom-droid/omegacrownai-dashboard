...
function validateVisualAssets(artifacts: GeneratedArtifact[], errors: GeneratedArtifactValidationIssue[]) {
  ...
  if (
    artifactFileSet.has("app/page.tsx") &&
    ...
    errors.push({
      ...
      file: "app/layout.tsx",
      message:
        "Generated Next.js App Router application requires app/layout.tsx.",
    });
  }

  function normalizeReferencedAsset(file: string) {
    return file
      .replace(/^\/+/, "")
      .replace(/^\.\//, "")
  ...
  function assertReferencedAssetExists(file: string, sourceFile: string) {
  ...
  if (htmlForReferencedAssets?.content) {
    const referencedAssets = Array.from(
      htmlForReferencedAssets.content.matchAll(/(?:src|href)=["']([^"']+\.(?:svg|png|jpg|jpeg|webp|gif|json|css|js))["']/gi)
    ).map((match) => match[1]);
...
export function validateGeneratedArtifacts(artifacts: GeneratedArtifact[]): GeneratedArtifactValidationResult {
  ...
  const isFinanceArtifact =
    artifactTextIncludes(artifacts, "index.html", ["Transactions", "Savings Rate", "Cash Flow"]) ||
