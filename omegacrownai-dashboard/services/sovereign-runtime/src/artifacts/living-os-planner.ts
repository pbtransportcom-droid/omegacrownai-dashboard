...
} from "../intelligence/build-spec.js";

export type LivingOSIndustry =
  | "bookstore"
  | "transportation"
  | "legal"
  | "healthcare"
...
export type LivingOSPagePlan = {
  id: string;
  name: string;
  route: string;
  role: LivingOSPageRole;
  purpose: string;
  audience: string[];
  sections: string[];
  actions: string[];
...
export type LivingOSModulePlan = {
  id: string;
  name: string;
  category:
    | "catalog"
...
export type LivingOSWorkflowPlan = {
  id: string;
  actor: string;
  name: string;
  steps: string[];
...
export type LivingOSDesignPlan = {
...
export type LivingOSProductionPlan = {
  version: "2.0";
  source: "authoritative-blueprint";
  industry: LivingOSIndustry;
  business: AuthoritativeBlueprint["business"];
  design: LivingOSDesignPlan;
  pages: LivingOSPagePlan[];
  modules: LivingOSModulePlan[];
  workflows: LivingOSWorkflowPlan[];
  architecture: {
    routes: string[];
    apiRoutes: string[];
    dataModels: string[];
  ...
  operatingSystem: {
    ...
    analyticsCenter: boolean;
    contentManager: boolean;
    aiCopilot: boolean;
  };
  delivery: AuthoritativeBlueprint["delivery"];
  compliance: AuthoritativeBlueprint["compliance"];
};
...
function detectIndustry(
  ...
  const explicitIndustry = String(
    blueprint.business?.industry || ""
  )
    .trim()
    .toLowerCase();

  const authoritativeMap: Record<
    string,
    LivingOSIndustry
  > = {
    bookstore: "bookstore",
  ...
  if (
    /\b(bookstore|book shop|bookseller|ebook|audiobook|author|book club)\b/.test(
      source
    )
  ) {
    return "bookstore";
  ...
  if (
    /\b(transportation|chauffeur|limousine|dispatch|fleet|airport transfer)\b/.test(
      source
    )
  ) {
  ...
  if (
    /\b(healthcare|medical|clinic|patient|doctor|hospital|pharmacy)\b/.test(
      source
    )
  ) {
    return "healthcare";
  ...
  if (
    /\b(restaurant|menu|reservation|kitchen|food ordering)\b/.test(source)
  ) {
    return "restaurant";
  }

  if (
    /\b(ecommerce|e-commerce|commerce|store|shop|checkout|product catalog)\b/.test(
      source
    )
  ) {
    return "commerce";
...
function inferPageRole(
  ...
  if (
    /dispatch|operations|operator|driver|staff|kitchen|clinical|workflow/.test(
      source
    )
  ) {
    return "operations";
  ...
  if (
    /customer|client|patient|account|portal|booking|orders|library|wishlist/.test(
      source
    )
  ) {
    return "customer";
...
function inferModuleCategory(
  featureName: string,
  featureCategory: string
): LivingOSModulePlan["category"] {
...
export function createLivingOSProductionPlan(
  blueprint: AuthoritativeBlueprint
): LivingOSProductionPlan {
  const industry = detectIndustry(blueprint);

  const blueprintRoutes = blueprint.architecture.routes.map(
    normalizeRoute
  );

  const pages: LivingOSPagePlan[] = blueprint.pages.map(
    (page, index) => {
      const route =
      ...
      const requiredFeatures = blueprint.features
        .filter((feature) => {
          ...
          return featureTokens.some((token) =>
            pageTokens.includes(token)
          );
        })
  ...
  const modules: LivingOSModulePlan[] = blueprint.features.map(
    (feature) => {
      const category = inferModuleCategory(
        feature.name,
        feature.category
      );
      ...
      const apiRoutes = blueprint.architecture.apiRoutes.filter(
        (route) => {
          const routeTokens = tokens(route);

          return featureTokens.some((token) =>
            routeTokens.includes(token)
          );
        }
      );
  ...
  const workflows: LivingOSWorkflowPlan[] =
    blueprint.workflows.map((workflow) => ({
      id:
        slug(`${workflow.actor}-${workflow.name}`) ||
        "business-workflow",
  ...
  return {
    ...
    design: {
      ...
      visualPersonality:
        blueprint.design.visualPersonality,
    ...
    operatingSystem: {
      ...
      customerPortal:
        /customer|client|patient|account|portal|orders|library/.test(
          combinedRequirements
        ),
      adminPortal: true,
      operationsCenter:
        /dispatch|workflow|driver|operator|staff|kitchen|clinical/.test(
          combinedRequirements
        ),
      notificationCenter:
        /notification|email|sms|message|alert/.test(
          combinedRequirements
        ),
      analyticsCenter:
        /analytics|report|metric|revenue|forecast/.test(
          combinedRequirements
        ),
      contentManager:
        /content|editor|cms|media|seo|promotion/.test(
          combinedRequirements
        ),
      aiCopilot: true,
    },
