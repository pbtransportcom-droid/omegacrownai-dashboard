...
function safeBrand(prompt: string, product: string) {
  ...
  const genericDescriptorPattern =
    /customer ready|customer-ready|strong copy|sections|calls to action|call to action|mobile layout|launch ready|launch-ready|polish|modern|customer|ready|copy/i;
...
function applyBookstoreCommerceSpecialization(files: any[], prompt: string, brand: string) {
  if (!isBookstorePrompt(prompt, brand)) return files;
  ...
  if (products) products.content = JSON.stringify(productData, null, 2);
...
function inferAssetIndustry(prompt: string, brand: string, buildSpec: any) {
  ...
  if (/(construction|contractor|renovation|estimate request|project gallery|roofing|plumbing|repair)/i.test(value)) return "construction";
  if (/(clinic|medical|doctor|health|appointment|patient)/i.test(value)) return "clinic";
  if (/(legal|law firm|lawyer|attorney|case review|consultation intake|practice areas)/i.test(value)) return "legal";
  if (/(beauty|salon|spa|hair|nails|makeup|skincare)/i.test(value)) return "beauty";
  if (/(restaurant|food|menu|reservation|catering|chef|dining)/i.test(value)) return "restaurant";
  if (/(transport|transportation|limo|airport|chauffeur|black car|fleet|rides|wedding service|corporate travel)/i.test(value)) return "transportation";

  return "";
}

function applyIndustryAssetSpecialization(files: any[], prompt: string, brand: string, buildSpec: any) {
  const industry = inferAssetIndustry(prompt, brand, buildSpec);

  const packs: Record<string, {
    domain: string;
    prompt: string;
    assets: Array<{ file: string; title: string; label: string; detail: string; bgA: string; bgB: string }>;
  }> = {
    construction: {
      domain: "construction",
