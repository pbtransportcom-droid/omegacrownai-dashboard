...
function automationBrand(
  ...
  if (
    !value ||
    /automation platform|workflow platform|custom business/i.test(
      value
    )
  ) {
...
export function renderAutomationLivingOS(
  plan: LivingOSProductionPlan
): LivingOSRenderedFile[] {
  if (plan.industry !== "automation") {
    throw new Error(
      `Automation renderer received ${plan.industry}`
    );
  }

  const brand =
    automationBrand(plan);
