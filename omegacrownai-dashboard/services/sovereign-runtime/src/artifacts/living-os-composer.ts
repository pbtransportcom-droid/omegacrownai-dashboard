...
import {
  getLivingOSRenderer,
  hasLivingOSRenderer,
  listLivingOSRendererRegistrations,
} from "./living-os-renderer-registry.js";

export type LivingOSRequirementEvidence = {
  requirement: string;
  category:
    | "page"
    | "feature"
...
export type LivingOSCompositionResult = {
  supported: boolean;
  industry: LivingOSIndustry;
  renderer: string | null;
  files: LivingOSRenderedFile[];
  traceability: {
    generatedAt: string;
    totalRequirements: number;
    implementedRequirements: number;
    coverage: number;
    evidence: LivingOSRequirementEvidence[];
  };
};
...
function evidenceFilesForTerms(
  files: LivingOSRenderedFile[],
  terms: string[]
) {
  return files
...
function createEvidence(
  requirement: string,
  category: LivingOSRequirementEvidence["category"],
  files: LivingOSRenderedFile[]
): LivingOSRequirementEvidence {
  const terms = importantTerms(requirement);
  const evidenceFiles =
    evidenceFilesForTerms(files, terms);
...
function createTraceability(
  ...
  const evidence: LivingOSRequirementEvidence[] = [];
...
export function composeLivingOSApplication(
  plan: LivingOSProductionPlan
): LivingOSCompositionResult {
  const registration =
    getLivingOSRenderer(plan.industry);
  ...
  const files =
    registration.renderer(plan);

  const traceability =
    createTraceability(plan, files);

  const traceabilityFile: LivingOSRenderedFile = {
    file: "living-os-traceability.json",
    title:
      "Living OS Requirement Traceability",
    type: "json",
