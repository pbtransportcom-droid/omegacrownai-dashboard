...
async function monitorGeneratedAppReadiness(
  ...
  persistGeneratedAppLifecycle(
    ...
    processAlive
      ? {
          status:
            getGeneratedAppManifest(projectId)?.status === "building"
              ? "building"
              : "starting",
          processAlive: true,
          portReachable: false,
    ...
    (code, signal) => {
      ...
      if (
        ...
        void (
          async () => {
            try {
              const repairResult: any =
                await orchestrateGeneratedAppRepair(
                  projectId
                );
              ...
              persistGeneratedAppLifecycle(
                ...
                {
                  automaticRepairTriggerFailed:
                    true,
                  automaticRepairTriggerError:
                    String(error),
                  checkedAt:
                    new Date().toISOString(),
                }
