...
type PlannerInput = {
  prompt: string;
  fallbackContract: PromptContract;
};
...
function getModel() {
  return String(
    process.env.OMEGACROWN_PLANNER_MODEL ||
...
function parseJsonObject(text: string) {
  ...
  throw new Error(
    "AI planner returned no parseable JSON object"
  );
}
...
function mergePromptContractPages(
  fallbackPages: any[],
  aiPages: any[]
) {
  const merged =
...
function mergePromptContractApis(
  fallbackApis: any[],
  aiApis: any[]
) {
  const merged =
...
function mergePromptContractModels(
  fallbackModels: any[],
  aiModels: any[]
) {
  const merged =
...
function mergePromptContractStrings(
...
function mergePromptContractWorkflows(
  fallbackWorkflows: any[],
  aiWorkflows: any[]
) {
  const merged =
    new Map<string, any>();
...
function normalizePlannerContract(
  input: any,
  fallback: PromptContract,
  ...
  return {
    ...
    pages:
      mergePromptContractPages(
        fallback.pages,
        pages
      ),

    apiRoutes:
      mergePromptContractApis(
        fallback.apiRoutes,
        apiRoutes
      ),

    models:
      mergePromptContractModels(
        fallback.models,
        models
      ),

    roles:
      mergePromptContractStrings(
        fallback.roles,
        input?.roles
      ),
...
export async function planPromptWithAI(
  input: PlannerInput
...
`;
  ...
  if (!response.ok) {
    throw new Error(
      `AI planner request failed: ${response.status} ${JSON.stringify(raw)}`
    );
  }
  ...
  if (!text) {
    throw new Error(
      "AI planner returned empty output"
    );
  }
