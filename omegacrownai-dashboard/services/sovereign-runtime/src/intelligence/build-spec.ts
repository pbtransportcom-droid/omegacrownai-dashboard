...
function detectBrand(prompt: string, fallback: string) {
  const explicit = prompt.match(
    /(?:called|named|brand(?:ed)? as|business name is|company name is)\s+([A-Z][A-Za-z0-9&' -]{1,70}?)(?=\s+(?:for|with|that|including|featuring|offering|serving|using|where|which)\b|\s*\(|[.,|]|$)/i
  );
...
export function createBuildSpec(input: {
  ...
  const transportationDomainIntent =
    /\b(transportation|transport service|transport services|limo|limousine|chauffeur|fleet|dispatch|vehicle dispatch|ride booking|passenger transportation|airport transportation|car service)\b/i.test(
      domainPrecedenceSource
    );

  const saasDomainIntent =
    /\b(saas|software as a service|multi[- ]tenant saas|subscription software|subscription software platform|cloud software platform|web application platform)\b/i.test(
      domainPrecedenceSource
    );
  ...
  const digitalIntelligenceSource =
    originalPrompt.toLowerCase();

  const explicitWorkflowAutomationIntent =
    /\b(workflow automation|automation platform|automation system|automate workflows?|automated workflows?|trigger(?: and action)? map|trigger rules?|action rules?|webhooks?)\b/i.test(
      digitalIntelligenceSource
    );
  ...
  const operationalWorkflowSignals = [
    ...
    pattern.test(digitalIntelligenceSource)
  ).length;

  const hasWorkflowOperatingContext =
    /\b(workflow|process|request|requests|task|tasks|case|cases|job|jobs|work item|work items|intake|operations)\b/i.test(
      digitalIntelligenceSource
    );

  const semanticWorkflowAutomationIntent =
    explicitWorkflowAutomationIntent ||
    (
  ...
  const explicitBuildIdentityMatch = originalPrompt.match(
    /\b(?:build|create)\s+(?:a|an)\s+(?:(?:production|customer|launch)[ -]?ready\s+)?(.{1,120}?\b(?:Operating System|Platform|Application|App|System))\b/i
  );

  const explicitBuildIdentity =
    explicitBuildIdentityMatch?.[1]?.trim() || "";

  const hasExplicitNamedBrand =
    /\b(?:called|named|brand(?:ed)?(?:\s+as)?|business name|company name)\b/i.test(
      originalPrompt
    );
  ...
  let brandName =
    explicitBuildIdentity && !hasExplicitNamedBrand
      ? explicitBuildIdentity
  ...
  if (
    ...
    const industryEvidence =
      `${productType} ${originalPrompt}`.toLowerCase();

    if (
      /transport|dispatch|chauffeur|black car|fleet|passenger|airport ride|limo/.test(
        industryEvidence
      )
    ) {
      industry = "transportation";
    } else if (
      /ecommerce|e-commerce|storefront|shopping cart|checkout|product catalog/.test(
        industryEvidence
      )
    ) {
      industry = "commerce";
    } else if (
      /clinic|medical|patient|healthcare|appointment/.test(
        industryEvidence
      )
    ) {
      industry = "clinic";
    } else if (
      /restaurant|menu|dining|catering/.test(
        industryEvidence
      )
    ) {
      industry = "restaurant";
    } else if (
      /legal|law firm|attorney|case intake/.test(
        industryEvidence
      )
    ) {
      industry = "legal";
    } else if (
      /construction|contractor|estimate request|project gallery/.test(
        industryEvidence
      )
    ) {
      industry = "construction";
    } else if (
      /beauty|salon|stylist/.test(
        industryEvidence
      )
    ) {
      industry = "beauty";
    } else if (
      /plumbing|plumber|drain|water heater/.test(
        industryEvidence
      )
    ) {
      industry = "plumbing";
    } else if (
      /bookstore|bookseller|ebook|audiobook/.test(
        industryEvidence
      )
    ) {
      industry = "bookstore";
  ...
  if (
    ...
    const strongSaasIdentity =
      /\bsaas\b|software as a service|multi[- ]tenant|workspace accounts?|subscription plans?|subscription billing/.test(
        saasEvidence
      );

    const operationalSaasIdentity =
      /workspace|organization|team members?|member roles?|projects?|tasks?|usage tracking|billing/.test(
        saasEvidence
      ) &&
      /subscription|tenant|workspace|organization/.test(
        saasEvidence
      );
  ...
  const appearsToUseGenericFallback =
    features.length > 0 &&
    features.every(
      (feature) =>
        genericFallbackFeatures.has(feature)
    );
  ...
  const professionalServicesSource =
    originalPrompt.toLowerCase();

  const professionalServicesExplicit =
    /\bprofessional services?\b/.test(
      professionalServicesSource
    );

  const professionalServicesFamilySignal =
    /\bconsult(?:ing|ant|ancy)\b|\badvisory\b|\baccounting\b|\baccountant\b|\bbookkeeping\b|\bbookkeeper\b|\bagency\b|\bclient delivery\b|\bengineering firm\b|\barchitecture firm\b|\barchitectural firm\b|\bit services\b|\bmanaged services\b|\bstatement of work\b|\bsow\b|\bretainer\b/.test(
      professionalServicesSource
    );

  const professionalServicesOperationsSignal =
    /\bclient portal\b|\bclients?\b|\bproposals?\b|\bengagements?\b|\bdeliverables?\b|\bmilestones?\b|\bstaff assignments?\b|\btime tracking\b|\btime entries\b|\binvoices?\b|\bproject delivery\b|\bdocument requests?\b/.test(
      professionalServicesSource
    );

  const professionalServicesExcluded =
    /\blaw firm\b|\blegal practice\b|\battorney\b|\bclinic\b|\bpatient\b|\bhealthcare\b|\btransportation\b|\bdispatch\b|\bfleet\b|\brestaurant\b|\bstorefront\b|\bshopping cart\b|\becommerce\b|\be-commerce\b|\btrading\b|\bbrokerage\b|\bbroker\b|\bsoftware as a service\b|\bmulti-tenant saas\b|\bsaas platform\b|\bsubscription software platform\b/.test(
      professionalServicesSource
    );

  const isProfessionalServicesBuild =
    !professionalServicesExcluded &&
    (
  ...
  if (isProfessionalServicesBuild) {
    ...
    const subtype =
      /\baccounting\b|\baccountant\b|\bbookkeeping\b|\bbookkeeper\b/.test(
        professionalServicesSource
      )
        ? "accounting and bookkeeping"
        : /\bagency\b|\bcreative agency\b|\bdigital agency\b/.test(
            professionalServicesSource
          )
          ? "agency and creative services"
          : /\bengineering firm\b|\barchitecture firm\b|\barchitectural firm\b/.test(
              professionalServicesSource
            )
            ? "engineering and architecture"
            : /\bit services\b|\bmanaged services\b|\bmanaged service provider\b|\bmsp\b/.test(
                professionalServicesSource
              )
    ...
    if (
      /marketing campaign system|customer-ready website|business web app|multi-tenant saas|saas project management|subscription software platform/i.test(
        productType
      )
    ) {
      productType =
  ...
  if (industry === "professional services") {
    const professionalSemanticSource =
      originalPrompt.toLowerCase();

    const professionalSubtype =
      /\baccounting\b|\baccountant\b|\bbookkeeping\b|\bbookkeeper\b/.test(
        professionalSemanticSource
      )
        ? "accounting and bookkeeping"
        : /\bagency\b|\bcreative agency\b|\bdigital agency\b|\bmarketing agency\b/.test(
            professionalSemanticSource
          )
          ? "agency and creative services"
          : /\bengineering firm\b|\barchitecture firm\b|\barchitectural firm\b|\bengineering\b|\barchitecture\b/.test(
              professionalSemanticSource
            )
            ? "engineering and architecture"
            : /\bit services\b|\bmanaged services\b|\bmanaged service provider\b|\bmsp\b/.test(
    ...
    const foreignProfessionalPages =
      new Set([
        "practice areas",
        "attorney profiles",
        "admin case review",
  ...
  const financeIntent =
    /\b(finance|financial|fintech|banking|bank|wealth|wealth management|portfolio|portfolios|ledger|accounting|accounts|transactions|transfers|budgeting|budgets|reconciliation)\b/i.test(
      originalPrompt
    ) ||
    /\b(finance|financial|fintech|banking|wealth|ledger|accounting)\b/i.test(
  ...
  if (financeIntent) {
    ...
    if (
      /\bwealth|portfolio|portfolios|holdings|advisor|advisors\b/i.test(
        originalPrompt
      )
    ) {
      productType =
  ...
  if (industry === "finance") {
    ...
    designPreset = {
      ...
      mood:
        "secure, precise, trustworthy, executive, data-focused",
      palette: {
        background: "#f8fafc",
        surface: "#ffffff",
        primary: "#0f172a",
      ...
      typography:
        "highly readable financial headings, precise numeric presentation, and clear account, transaction, transfer, risk, and compliance labels",
      layout:
        "financial dashboard, account overview, transaction operations, secure transfer workspace, budgeting, reporting, risk, compliance, audit, and administration",
      heroStyle:
        "secure finance operating-platform hero focused on account visibility, transaction control, transfer safety, governance, and measurable oversight",
      sectionStyle:
        "structured financial operations sections with account summaries, transaction tables, transfer controls, budget panels, reporting, risk queues, compliance review, audit history, and executive oversight",
      imageDirection:
        "professional financial operations imagery emphasizing secure account management, transaction oversight, trusted transfer workflows, reporting, risk controls, compliance, and financial governance",
      motionDirection:
        "restrained professional transitions with subtle dashboard feedback, transaction and transfer status changes, validation states, and low-distraction financial workflow motion",
    };
  }
  ...
  if (industry === "professional services") {
    const requestedProfessionalBrand =
      typeof input.productName === "string"
        ? input.productName.trim()
        : "";
  ...
  if (industry === "professional services") {
    ...
    designPreset = {
      ...
      mood:
        "executive, trustworthy, structured, client-focused, polished",
      palette: {
        background: "#f8fafc",
        surface: "#ffffff",
        primary: "#0f172a",
      ...
      typography:
        "executive business headings, highly readable operational copy, clear client, engagement, project, billing, and workflow labels",
      layout:
        "executive application shell, client relationship overview, intake and proposal workflow, engagement and project delivery panels, client portal, time and billing operations, analytics, and administration",
      heroStyle:
        "professional services operating-platform hero focused on client relationships, engagements, project delivery, and measurable business outcomes",
      sectionStyle:
        "clean operational cards, client and engagement panels, project delivery sections, document workflows, billing operations, analytics, and administrative controls",
      imageDirection:
        "professional advisory teams, client collaboration, executive meetings, project delivery, documents, analytics, and business operations",
      motionDirection:
        "subtle professional transitions, clear workflow state changes, restrained executive interaction",
    };
  }
  ...
  const explicitBookstoreIdentity =
    /\b(bookhaven|bookstore|book store|book shop|bookseller|ebook|ebooks|audiobook|audiobooks|book catalog|book lover|book lovers)\b/i.test(
      bookstoreIdentitySource
    );
  ...
  if (industry === "bookstore") {
    ...
    const bookstorePageObjects =
      bookstorePages.map(
        (name, index) => ({
          name,
          purpose:
            `Support the bookstore customer journey through ${name}.`,
          requiredSections: [
            `${name} introduction`,
            `${name} primary content`,
            `${name} customer action`,
          ...
          route:
            bookstoreRoutes[index] || "/",
        })
      );

    const bookstoreFeatureObjects =
      bookstoreFeatures.map(
        (name) => ({
          name,
          category:
            /admin|inventory|customer|order|promotion/i.test(name)
              ? "admin"
