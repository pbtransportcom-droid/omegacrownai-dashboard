...
import { listSoundRoomSessions } from "@/lib/sugent/sound-room/soundRoomEngine";

export default async function SoundRoomPage({
  params,
}: {
  ...
  const data = company ? await listSoundRoomSessions(company.id) : null;
  ...
  return (
    <main className="space-y-6">
      ...
      <section className="rounded-3xl border border-border bg-panel/70 p-6">
        ...
        <p className="mt-5 text-xs uppercase tracking-[0.25em] text-pink-300">
          AI Sound Designer&apos;s Room · Phase 32
        </p>

        <h1 className="mt-3 text-4xl font-black text-white">
          Sound Designer&apos;s Room · {project?.name || "Project"}
        </h1>

        <p className="mt-3 max-w-3xl text-sm leading-7 text-muted">
          Multi-agent audio critique for voice, music, sound effects, mixing, accessibility, brand tone, and production-grade polish.
        </p>
      </section>
      ...
      {company && data ? (
        <>
          ...
          <section className="rounded-3xl border border-border bg-panel/70 p-5">
            <h2 className="text-xl font-black text-white">Start Sound Designer&apos;s Room</h2>

            <form
              action={`/api/company/${company.id}/sound-room`}
              method="POST"
            ...
            >
              ...
              <input
                name="topic"
                placeholder="Topic, e.g. Premium voice/music/SFX direction for launch video"
                className="rounded-xl border border-border bg-slate-950 px-4 py-3 text-sm text-white outline-none"
              />

              <button className="rounded-xl bg-pink-600 px-5 py-3 text-sm font-black text-white hover:bg-pink-500">
                Start Audio Critique
              </button>
            </form>
          </section>
          ...
          <section className="space-y-4">
            {data.sessions.length ? (
                  ...
                  <div key={session.id} className="rounded-3xl border border-border bg-panel/70 p-5">
                    <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-xs text-pink-300">{session.status} · {session.projectType}</p>
                        <p className="mt-1 font-mono text-[11px] text-muted">{session.id}</p>
                      </div>

                      <div className="flex flex-wrap gap-2">
                        <form action={`/api/company/${company.id}/sound-room/${session.id}/next-round`} method="POST">
                          <button className="rounded-xl border border-pink-400/30 bg-pink-500/10 px-3 py-2 text-xs font-black text-pink-100 hover:bg-pink-500/20">
                            Run Next Round
                          </button>
                        </form>

                        <form action={`/api/company/${company.id}/sound-room/${session.id}/apply-plan`} method="POST">
                        ...
                        <a
                          href={`/api/company/${company.id}/sound-room/${session.id}`}
                    ...
                    {latestRound ? (
                      <div className="mt-5 grid gap-4 xl:grid-cols-6">
                        <Panel title="Audio Snapshot" value={JSON.stringify(latestRound.audioJson, null, 2)} wide />
                        {["voice", "music", "sfx", "mix"].map((agent) => {
                        ...
                        <Panel title="Audio Plan" value={JSON.stringify(latestRound.audioPlanJson || [], null, 2)} wide />
                      </div>
                    ) : (
              ...
              <div className="rounded-xl border border-border bg-black/20 p-4 text-sm text-muted">
                No Sound Designer&apos;s Room sessions yet.
              </div>
            )}
          </section>
        </>
...
function Panel({
  ...
  return (
    <div className={`rounded-2xl border border-border bg-black/20 p-4 ${wide ? "xl:col-span-3" : "xl:col-span-1"}`}>
      <div className="text-xs font-black uppercase tracking-[0.16em] text-pink-300">
        {title}
      </div>
